...
Media Specification 現行版本新增 Organization 一節，其中 Flow（閱讀順序、視線流向）、Relationship（對話、人物互動）在概念名稱上與本節「對話框閱讀順序」「對話與角色一致性」相近。

是否可以直接引用 Organization 之 Flow、Relationship 作為上述兩項之資料來源，尚待確認——Media Specification 版本紀錄並未說明 Organization 之新增與本節有關，此對應關係目前僅為推論，不視為已成立。

Speaker（說話者）、Dialogue（對話內容）、Speech Bubble（對話框）三者，Organization 中無對應之正式命名欄位，此缺口尚未補上。

在此事項確認前，維持原規則：若對話內容需要固定欄位（例如角色 ID、對話框座標）以利重複使用，應由使用者或《Media Specification》另行擴充，本模式不自行新增資料欄位。
...
- 第六節、第七節：「Visual」更名為「Presentation」，同步 Media Specification v2.2 之章節更名（原第五節 Visual → 新第六節 Presentation）。
- 第十二節：新增【待決事項】區塊，記錄 Media Specification 新增之 Organization（Flow、Relationship）與本節對話框、人物互動概念可能之對應關係；此對應關係尚未確認成立，規則本身未變動，僅補充待確認事項供後續參考。

本次調整不改變任何檢查項目、判斷邏輯或優先順序之實質內容，僅同步用詞與記錄待確認事項。
