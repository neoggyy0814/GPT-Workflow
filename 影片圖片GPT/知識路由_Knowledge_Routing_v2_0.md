...
- 全文「Personal Workflow GPT Specification」統一更名為「Runtime」，共 3 處（第一節、第六節、第八節優先順序清單），對應該名稱確認即為 Runtime 舊稱。
- 第八節「路由優先順序」由原本 5 項（User Intent > Personal Workflow GPT Specification > Knowledge Routing > Mode > Specification）修正為與 Runtime 第十七節一致之 5 項排序：User Intent > Runtime > Knowledge Routing > Specification > Mode。原順序中 Mode 排在 Specification 之前，與 Runtime 定義相反，本次一併修正。

本次調整僅為命名統一與優先序對齊，不改變本文件既有之任務判斷、模式路由、規格路由邏輯。
